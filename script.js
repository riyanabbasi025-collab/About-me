(() => {
  ...
  function openDiscordSetup() {
    ...
    openModal(`<p class="eyebrow">DISCORD // MANUAL PROFILE</p><h2 id="modal-title">Edit your profile card</h2>
      ...
      <div class="form-field"><label>AVATAR DECORATION — OPTIONAL URL OR LOCAL IMAGE</label><div class="file-row"><input id="f-discord-decoration" value="${esc((p.decoration || '').startsWith('data:') ? '' : (p.decoration || ''))}" placeholder="Transparent PNG / WebP"><label class="btn ghost file-picker">CHOOSE DECOR<input id="f-discord-decoration-file" type="file" accept="image/png,image/webp,image/gif"></label><button class="btn ghost" id="clear-discord-decoration" type="button">REMOVE DECOR</button></div><p class="muted-note">Use a transparent image sized to frame the avatar. REMOVE DECOR clears any saved decoration completely.</p></div>
